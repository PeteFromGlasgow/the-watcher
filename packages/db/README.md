...
This package contains the database migrations and seeds for The Watcher.
...
```bash
pnpm --filter @watcher/db run db:migrate:make <migration_name>
```
...
```bash
pnpm --filter @watcher/db run db:migrate
```
...
```bash
pnpm --filter @watcher/db run db:seed
